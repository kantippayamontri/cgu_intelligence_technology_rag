...
You can invoke an agent by passing an update to its [`State`](about:/oss/python/langgraph/graph-api#state). All agents include a [sequence of messages](about:/oss/python/langgraph/use-graph-api#messagesstate) in their state; to invoke the agent, pass a new message:
